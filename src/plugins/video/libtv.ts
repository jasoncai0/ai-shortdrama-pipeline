    ...
    return {
      ...
      generate: async (req): Promise<readonly AssetRef[]> => {
        ...
        const pushRef = (ref: AssetRef | undefined, what: string): void => {
          ...
          if (!canvasName) {
            throw providerError(
              ...
              'Stills must be generated by the libtv image adapter on the same canvas.',
            )
          }
        ...
        if (req.mode === 'singleImage2video' || req.mode === 'frames2video') {
          pushRef(req.firstFrame, 'first frame')
        }
        if (req.mode === 'frames2video') pushRef(req.lastFrame, 'last frame')
        if (req.mode === 'image2video' || req.mode === 'mixed2video') {
          for (const ref of req.refs ?? []) pushRef(ref, 'reference image')
        }
